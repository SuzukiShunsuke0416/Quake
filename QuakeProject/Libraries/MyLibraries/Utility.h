...
/*
	※１　角度補完値のゴール値候補について
	まず前提として、補完値を元に計算される値(mMoveWayAngle)は
	-2piから2piの間でループするように処理が書かれている。

	例えば、↑(0度)から→(270度)へ滑らかに遷移させる時は反時計回りに大回りしてしまう。
	それを改善するため→の値を３パターン用意する。
	１．270度　２．630度(基本値+pi)　３．-90度(基本値-pi)
	この中から現在値（0度）に一番近い値を選び、そこへ向かうように計算する。
	今回の場合は0度から-90度へ動くので、最短回転(時計回り)で済む。

	４つ目の値 -450度が存在する理由は、現在値が-360度(実際には-359.999…)になっている時に
	-2piループ処理に入りきらない状態で→が選択された際に-90に向かう現象(時計回りに大回り)
	が起きてしまうため、これを防ぐために-450度という値が存在している。
*/